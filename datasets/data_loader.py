...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    if 'mnist' == dataset:
        ...
        tvmnist_trn = PathMNIST(root=path, split="train", download=False, as_rgb=True)
        tvmnist_tst = PathMNIST(root=path, split="test", download=False, as_rgb=True)
        ...
        # import pdb
        # pdb.set_trace()
        tvmnist_trn = OrganAMNIST(root=path, split="train", download=False, as_rgb=True)
        tvmnist_tst = OrganAMNIST(root=path, split="test", download=False, as_rgb=True)
        trn_data = {'x': tvmnist_trn.imgs, 'y': tvmnist_trn.labels.tolist()}
        tst_data = {'x': tvmnist_tst.imgs, 'y': tvmnist_tst.labels.tolist()}
        # compute splits
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
        ...
        tvmnist_trn = TissueMNIST(root=path, split="train", download=False, as_rgb=True)
        tvmnist_tst = TissueMNIST(root=path, split="test", download=False, as_rgb=True)
        trn_data = {'x': tvmnist_trn.imgs, 'y': tvmnist_trn.labels.tolist()}
        tst_data = {'x': tvmnist_tst.imgs, 'y': tvmnist_tst.labels.tolist()}
        # compute splits
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
...
def get_transforms(resize, pad, crop, flip, normalize, extend_channel):
    ...
    if extend_channel is not None:
        trn_transform_list.append(transforms.Lambda(lambda x: x.repeat(extend_channel, 1, 1)[:3]))
        tst_transform_list.append(transforms.Lambda(lambda x: x.repeat(extend_channel, 1, 1)[:3]))
