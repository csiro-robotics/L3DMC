...
from mixed_curvature_distillation_latest import MCD, DistilationLoss
...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=60, lr=0.5, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 eval_on_train=False, logger=None, exemplars_dataset=None, alpha=1, beta=1):
        ...
        self.alpha = alpha
        self.beta = beta
        self.mcd_criterion = MCD(512, 512, 128).to(self.device)
        self.kl_div = DistilationLoss()
        
        # L3DMC is expected to be used with exemplars. If needed to be used without exemplars, overwrite here the
        # `_get_optimizer` function with the one in LwF and update the criterion
        have_exemplars = self.exemplars_dataset.max_num_exemplars + self.exemplars_dataset.max_num_exemplars_per_class
        if not have_exemplars:
            warnings.warn("Warning: L3DMC is expected to use exemplars.")
    ...
    def extra_parser(args):
        ...
        # Sec. 4. " allowing L3DMC to balance between CE and distillation loss."
        parser.add_argument('--alpha', default=1.0, type=float, required=False,
                            help='Alpha: weight for KD (default=%(default)s)')
        parser.add_argument('--beta', default=1.0, type=float, required=False,
                            help='Beta: weight for FD (default=%(default)s)')
        return parser.parse_known_args(args)


    # Algorithm 1: L3DMC NCM Classify
    ...
    # Algorithm 2: L3DMC Incremental Train
    def train_loop(self, t, trn_loader, val_loader):
        ...
        # Algorithm 3: L3DMC Update Representation
        ...
        # Algorithm 4: ConstructExemplarSet and Algorithm 5: ReduceExemplarSet
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            feats_old = None
            if t > 0:
                outputs_old, feats_old = self.model_old(images.to(self.device), return_features=True)
            # Forward current model
            # import pdb; pdb.set_trace()
            outputs, feats = self.model(images.to(self.device), return_features=True)
            loss = self.criterion(t, feats, outputs, targets.to(self.device), feats_old, outputs_old)
            # Backward
    ...
    def criterion(self, t, features, outputs, targets, features_old=None, outputs_old=None):
        """Returns the loss value"""
        
        if type(outputs[0]) == dict:
            outputs_ = torch.cat([o for o in outputs], dim=1)
        else:
            outputs_ = torch.cat(outputs, dim=1)

        # Classification loss
        loss = torch.nn.functional.cross_entropy(outputs_, targets, weight=self.weights)

        # Distillation loss
        if t > 0:
            
            loss += self.alpha * self.kl_div(torch.cat([o for o in outputs[:t]], dim=1), torch.cat([o_ for o_ in outputs_old[:t]], dim=1).detach()).mean()
            
            loss += self.beta * self.mcd_criterion(features, features_old.detach())
